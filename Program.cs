        ...
        static int Main(string[] args)
        {
            using (Logger.Root = new Logger())
            {
                ...
                try
                {
                    ...
                    {
                        Logger.Root.PrintInfo("CommandLine", Environment.CommandLine);
                        Logger.Root.PrintInfo("Start time", start);
                    }
                    command.Run();
                    ...
                    if (command.GetType() != typeof(CommandHelp))
                    {
                        Logger.Root.PrintInfo("Total time", et);
                        Logger.Root.PrintInfo("End time", end);
                ...
                {
                    Logger.Root.Error("{0}: {1}", error.InnerException.GetType().Name, error.InnerException.Message);
                }
                catch (Exception error)
                {
                    Logger.Root.Error("{0}: {1}", error.GetType().Name, error.Message);
                }
                return 2;
            }
        }
